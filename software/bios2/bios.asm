...
JIFFY_VIA_TIMER_LOAD = 20000   ; this is the value for 1MHZ / 50 ticks per second
; constants for LCD
LCD_E  = %10000000
LCD_RW = %01000000
LCD_RS = %00100000
...
RAMTOP = $30 ; store the page of the last RAM ($30 is the low adress)

JTIME = $A0 ; to $A2 three bytes jiffy time, higher two bytes of the 3 bytes of the 1/50 secs of a day. 24h * 60m * 60s * 50, 4.320.000 ticks per day

IN_READ = $80
IN_WRITE = $81
TEMP_VEC = $32 ; store a temporary vector, like the address to the string to output, $32 low, $33 hi
; Stack  $0100.. $01ff
SPAGE = $0100
; Bios data
BIOSPAGE = $0200
IRQ_SRV =  $0214    ; $0214 LOW byte, $0215 HIGH byte for a external irq service routine
NMI_SRV =  $0216    ; $0216 LOW byte, $0217 HIGH byte for a external nmi service routine
RTI_SRV =  $0218    ; every user irq or nmi routine should call this for returning from interrupt, jmp (RTI_SRV)
IN_BUF_LEN = $0F    ; length of input buffer
IN_BUFFER = $0280   ; 16 bytes of input buffer
